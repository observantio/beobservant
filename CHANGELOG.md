...
* Fixed the Grafana dashboard sync confirmation so syncing datasource visibility no longer re-submits dashboard creation and emits a false 409, and made Loki Log Results auto-height so compact rows no longer force a nested scrollbar.
...
* Moved watchdog proxy I/O off the event loop, batched Tempo trace hydration, cached agent activity lookups, and kept the generated watchdog OpenAPI snapshot current.
...
* Updated chart README quickstart/profile docs to use a cleaner Kubernetes-focused guide and added the Kubernetes visual asset.
* Refined login UI styling and auth form presentation for clearer, cleaner sign-in UX.
* Added watchdog entrypoint test coverage for SSL-enabled runtime listener startup (`__main__` path).
