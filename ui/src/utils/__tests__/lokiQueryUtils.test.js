...
describe('lokiQueryUtils', () => {
  ...
  it('deduplicates and sorts label values', () => {
    const values = normalizeLabelValues('service', ['service="api"', 'service="web"', 'service="api"'])
  ...
  it('computes top terms with fallback and extracts volumes', () => {
    ...
    const volume = getVolumeValues({ data: { result: [{ values: [['1', '3'], ['2', '4']] }] } })
  ...
  it('builds selectors and fallback volume buckets', () => {
    ...
    expect(buildSelectorFromFilters([{ label: 'service', value: 'api' }])).toBe('{service="api"}')

    const fallback = buildFallbackVolume({ data: { result: [{ values: [['1000000000', '1']] }] } }, 0)
