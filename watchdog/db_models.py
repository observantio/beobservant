"""
All SQLAlchemy models for the main server, defining schema for tenants, users, groups, permissions, Grafana resources,
...
class ApiKeyShare(Base):
    ...
    api_key_id: Mapped[str] = mapped_column(
        String, ForeignKey("user_api_keys.id", ondelete="CASCADE"), nullable=False, index=True
    )
    owner_user_id: Mapped[str] = mapped_column(
        String, ForeignKey(_FK_USERS, ondelete="CASCADE"), nullable=False, index=True
    )
    shared_user_id: Mapped[str] = mapped_column(
        String, ForeignKey(_FK_USERS, ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    __table_args__ = (UniqueConstraint("api_key_id", "shared_user_id", name="uq_api_key_shares_key_user"),)
...
class HiddenApiKey(Base):
    ...
    __table_args__ = (UniqueConstraint("tenant_id", "user_id", "api_key_id", name="uq_hidden_api_keys_user_key"),)
