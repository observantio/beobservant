...
NO_AUTH_CASES: list[tuple[str, str, str, dict[str, Any] | list[Any] | None, dict[str, Any] | None, int]] = [
    ...
    ("auth_user_create", "POST", "/api/auth/users", {"username": "u", "email": "u@example.com", "password": "password123"}, None, 401),
    ...
    ("resolver_jobs_create", "POST", "/api/resolver/analyze/jobs", {"start": 1, "end": 2, "services": ["api"], "log_query": "{service=\"api\"}"}, None, 401),
    ...
    ("alertmanager_channel_create", "POST", "/api/alertmanager/channels", {"name": "c", "type": "email", "config": {}}, None, 401),
    ("alertmanager_silences", "GET", "/api/alertmanager/silences", None, None, 401),
    ("alertmanager_silence_create", "POST", "/api/alertmanager/silences", {"id": "sil-1", "visibility": "private"}, None, 401),
    ("alertmanager_incidents", "GET", "/api/alertmanager/incidents", None, None, 401),
    ("alertmanager_incident_patch", "PATCH", "/api/alertmanager/incidents/inc-1", {"status": "acknowledged"}, None, 401),
]
...
INVALID_TOKEN_CASES: list[tuple[str, str, str, dict[str, Any] | list[Any] | None, dict[str, Any] | None, int]] = [
    ...
    ("invalid_resolver_create", "POST", "/api/resolver/analyze/jobs", {"start": 1, "end": 2, "services": ["api"], "log_query": "{service=\"api\"}"}, None, 401),
    ...
    ("invalid_alert_channel_create", "POST", "/api/alertmanager/channels", {"name": "c", "type": "email", "config": {}}, None, 401),
    ("invalid_alert_rule_create", "POST", "/api/alertmanager/rules", {"name": "r", "expr": "up == 0"}, None, 401),
    ("invalid_alert_silence_create", "POST", "/api/alertmanager/silences", {"id": "sil-invalid", "visibility": "private"}, None, 401),
]


INTERNAL_AND_PUBLIC_CASES: list[tuple[str, str, str, dict[str, str] | None, dict[str, Any] | list[Any] | None, dict[str, Any] | None, int]] = [
    ...
    ("internal_wrong_header", "POST", "/api/internal/otlp/validate", {"X-Internal-Token": "wrong"}, {"token": "x"}, None, 403),
    ("internal_empty_payload_token", "POST", "/api/internal/otlp/validate", {"X-Internal-Token": "wrong"}, {"token": ""}, None, 403),
    ("internal_whitespace_payload_token", "POST", "/api/internal/otlp/validate", {"X-Internal-Token": "wrong"}, {"token": "   "}, None, 403),
    ("internal_query_deprecated_missing_header", "GET", "/api/internal/otlp/validate", None, None, {"token": "x"}, 422),
    ("internal_query_deprecated_wrong_header", "GET", "/api/internal/otlp/validate", {"X-Internal-Token": "wrong"}, None, {"token": "x"}, 403),
    ...
    ("oidc_authorize_without_oidc", "POST", "/api/auth/oidc/authorize-url", None, {"redirect_uri": "https://app.example.com/cb", "state": "s"}, None, 400),
    ("oidc_exchange_without_oidc", "POST", "/api/auth/oidc/exchange", None, {"code": "c", "redirect_uri": "https://app.example.com/cb"}, None, 400),
]
...
def test_workflow_public_and_internal_boundary_matrix(
    ...
    monkeypatch.setattr(internal_router.internal_service._auth_service, "validate_otlp_token", lambda *_args, **_kwargs: "org-x")
