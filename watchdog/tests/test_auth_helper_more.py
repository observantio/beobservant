...
def test_audit_helpers_and_filters(monkeypatch):
    ...
    assert (
        auth_helper.sanitize_resource_id("https://host/path?token=abc&x=1")
        == "https://host/path?token=%5BREDACTED%5D&x=1"
    )
    ...
    actor = User
    superuser = TokenData(
        user_id="u1", username="su", tenant_id="t1", org_id="o1", role=Role.ADMIN, permissions=[], is_superuser=True
    )
...
def test_role_permission_admin_and_rate_limit_delegate(monkeypatch):
    ...
    assert auth_helper.perms_check(
        TokenData(user_id="u1", username="x", tenant_id="t1", org_id="o1", role=Role.USER, permissions=["a", "b"])
    ) == {"a", "b"}
    assert (
        auth_helper.is_admin_check(
            TokenData(user_id="u2", username="x", tenant_id="t1", org_id="o1", role=Role.ADMIN, permissions=[])
        )
        is True
    )
