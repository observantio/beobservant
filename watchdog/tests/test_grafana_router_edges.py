...
async def test_dashboard_routes_cover_success_and_failure_paths(monkeypatch):
    ...
    monkeypatch.setattr(
        dashboards.proxy, "build_dashboard_search_context", lambda *_args, **_kwargs: {"uid_db_dashboard": object()}
    )
    ...
    assert (
        await dashboards.create_dashboard(
            payload, visibility="private", shared_group_ids=["g1"], current_user=current_user, db="db"
        )
    )["uid"] == "created"
    assert (await dashboards.save_dashboard_from_grafana_ui(payload, current_user=current_user, db="db"))[
        "updated"
    ] is True

    monkeypatch.setattr(
        dashboards.proxy, "build_dashboard_search_context", lambda *_args, **_kwargs: {"uid_db_dashboard": None}
    )
    ...
    assert await dashboards.delete_dashboard("dash-1", current_user=current_user, db="db") == {
        "status": "success",
        "message": "Dashboard dash-1 deleted",
    }
    with pytest.raises(HTTPException) as exc:
    ...
    assert await dashboards.hide_dashboard(
        "dash-1", GrafanaHiddenToggleRequest(hidden=False), current_user=current_user, db="db"
    ) == {"status": "success", "hidden": False}
...
async def test_datasource_routes_cover_success_and_failure_paths(monkeypatch):
    ...
    assert (await datasources.datasource_query(payload, current_user=current_user, db="db"))["queried"]["queries"][0][
        "expr"
    ] == "up"
    ...
    assert (
        await datasources.create_datasource(
            create_payload, visibility="private", shared_group_ids=["g1"], current_user=current_user, db="db"
        )
    )["uid"] == "created"
    assert (
    ...
    assert await datasources.delete_datasource("ds-1", current_user=current_user, db="db") == {
        "status": "success",
        "message": "Datasource ds-1 deleted",
    }
    with pytest.raises(HTTPException) as exc:
        await datasources.delete_datasource("missing", current_user=current_user, db="db")
    assert exc.value.status_code == 404

    assert await datasources.hide_datasource(
        "ds-1", GrafanaHiddenToggleRequest(hidden=False), current_user=current_user, db="db"
    ) == {"status": "success", "hidden": False}
...
async def test_folder_routes_cover_success_and_failure_paths(monkeypatch):
    ...
    assert await folders.get_folders(show_hidden=True, current_user=current_user, db="db") == [
        {"uid": "folder-1", "show_hidden": True}
    ]
    ...
    assert (
        await folders.create_folder(
            create_payload, visibility="group", shared_group_ids=["g1"], current_user=current_user, db="db"
        )
    )["uid"] == "created-folder"
    assert await folders.delete_folder("folder-1", current_user=current_user, db="db") == {
        "status": "success",
        "message": "Folder folder-1 deleted",
    }
    with pytest.raises(HTTPException) as exc:
    ...
    assert await folders.hide_folder(
        "folder-1", GrafanaHiddenToggleRequest(hidden=False), current_user=current_user, db="db"
    ) == {"status": "success", "hidden": False}
