...
def test_group_helper_functions_and_access_rules(monkeypatch):
    ...
    assert (
        group_ops._can_access_group(group, actor_user_id=member.id, actor_role="user", actor_is_superuser=False) is True
    )
    assert (
        group_ops._can_access_group(group, actor_user_id=other.id, actor_role="admin", actor_is_superuser=False) is True
    )
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: (_ for _ in ()).throw(HTTPException(status_code=502, detail="boom")),
    )
