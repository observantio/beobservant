...
class OidcServiceTests(unittest.TestCase):
    def test_is_fresh_respects_cache_ttl(self):
        ...
        with patch.object(_oidc_module.time, "time", return_value=100):
    ...
    def test_build_authorization_url_contains_required_params(self):
        ...
        with (
            patch.object(
                service, "_get_well_known", return_value={"authorization_endpoint": "https://idp.example/auth"}
            ),
...
def test_oidc_pkce_mismatch_is_rejected():
    service = OIDCService()
    with (
        patch.object(service, "_get_well_known", return_value={"authorization_endpoint": "https://idp.example/auth"}),
        patch.object(_oidc_module.config, "OIDC_CLIENT_ID", "client-1"),
        patch.object(_oidc_module.config, "OIDC_SCOPES", "openid profile"),
    ):
