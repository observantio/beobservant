...
def test_create_user_policy_and_external_branches(monkeypatch):
    ...
    with pytest.raises(HTTPException, match="Creator account"):
        user_ops.create_user(
            ...
            UserCreate(
                username="badcreator",
                email="badcreator@example.com",
                password="password1",
                full_name="Bad Creator",
                org_id="default-org",
            ),
            "t1",
    ...
    created_external = user_ops.create_user(
        external_service,
        UserCreate.model_construct(
            username="externalnew",
            email="externalnew@example.com",
            password="",
            full_name="External New",
            org_id="default-org",
            role=Role.USER,
            group_ids=[],
            is_active=True,
    ...
    with pytest.raises(ValueError, match="provisioning failed"):
        user_ops.create_user(
            failing_external,
            UserCreate.model_construct(
                username="externalfail",
                email="externalfail@example.com",
                password="",
                full_name="External Fail",
                org_id="default-org",
                role=Role.USER,
                group_ids=[],
                is_active=True,
...
def test_update_user_and_delete_user_paths(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: prune_calls.append((args, kwargs)),
    )
    monkeypatch.setattr(
        user_ops, "_propagate_removed_member_group_shares", lambda **kwargs: propagate_calls.append(kwargs)
    )
    service = _service()
    ...
    updated = user_ops.update_user(
        service, user.id, UserUpdate(username="renamed", org_id="new-org", group_ids=[]), "t1", updater_id=admin.id
    )
...
def test_update_user_permissions_paths(monkeypatch):
    ...
    assert {permission.name for permission in db.query(User).filter_by(id=user.id).first().permissions} == {
        perm.name,
        admin_perm.name,
    }
