...
async def get_services(current_user: TokenData = Depends(require_permission(Permission.READ_TRACES))) -> List[str]:
...
async def get_operations(service: str, current_user: TokenData = Depends(require_permission(Permission.READ_TRACES))) -> List[str]:
