...
def get_accessible_datasource_uids(
    ...
    if group_ids:
        conditions.append(
            and_(GrafanaDatasource.visibility == "group", GrafanaDatasource.shared_groups.any(Group.id.in_(group_ids)))
        )
...
async def enforce_datasource_query_access(
    ...
    if id_match:
        ...
        ds = check_datasource_access_by_id(db, ds_id, user_id, tenant_id, group_ids, require_write=False)
        if not ds:
            maybe_registered = (
                db.query(GrafanaDatasource)
    ...
    for datasource_uid in referenced_uids:
        ds = check_datasource_access(db, datasource_uid, user_id, tenant_id, group_ids, require_write=False)
        if ds:
            continue
...
async def get_datasources(
    ...
    for d in all_datasources:
        ...
        is_unregistered_safe_system = allow_system and uid_val not in all_registered_uids and _is_safe_system_datasource(d)
        if uid_val not in accessible and not is_unregistered_safe_system:
            continue
...
async def update_datasource(
    ...
    try:
        ...
        if isinstance(exc, GrafanaAPIError) and exc.status in {409, 412} and requested_name:
            ...
            retry_payload = datasource_update.model_copy(update={"name": internal_name})
            try:
                result = await service.grafana_service.update_datasource(uid, retry_payload)
            except Exception as retry_exc:
                service._raise_http_from_grafana_error(retry_exc)
                return None
        else:
