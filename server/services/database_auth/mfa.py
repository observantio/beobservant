...
def _consume_recovery_code(service, db_user, code: str) -> bool:
...
def enroll_totp(service, user_id: str) -> Dict[str, str]:
    ...
    with get_db_session() as db:
        ...
        db.flush()
        uri = pyotp.TOTP(secret).provisioning_uri(name=user.email or user.username, issuer_name="Be Observant")
...
def disable_totp(service, user_id: str, current_password: Optional[str] = None, code: Optional[str] = None) -> bool:
    with get_db_session() as db:
        ...
        if not user or not user.mfa_enabled:
            return False
        verified = (current_password and service.verify_password(current_password, user.hashed_password)) or (
...
def _mfa_setup_challenge(service, user) -> dict:
    setup_token = create_mfa_setup_token_op(service, user)
    return {service._MFA_SETUP_RESPONSE: True, "setup_token": setup_token.access_token if setup_token else None}


def _needs_mfa_setup(service, user) -> bool:
