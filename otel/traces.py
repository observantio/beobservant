...
def send(spans):
    ...
    req = Request(
        f"http://{ENDPOINT}/v1/traces", data=payload, headers={"Content-Type": "application/json"}, method="POST"
    )
    try:
...
def make_trace():
    ...
    def add(name, svc, dur, parent=None, error=False, **attrs):
        if len(spans) >= max_spans:
            return None
        sp = Span(
            trace_id, name, svc, dur, parent_id=parent, error=error, attrs={"env": env, "cloud.region": region, **attrs}
        )
        spans.append(sp)
        return sp.span_id
    ...
    if route.startswith("/api/product") or route == "/api/search":
        ...
        if cs:
            add(
                pick(["MGET products:*", "SELECT * FROM products LIMIT 50"]),
                pick(["redis-cache", "postgres-replica-1"]),
                rand(5, 40),
                parent=cs,
                **{"db.operation": "SELECT"},
            )
        ...
        if os_ and not oerr:
            ...
            if ps:
                add("POST /v1/charges", prov, latency(250, 450, 30), parent=ps, error=perr, **{"http.method": "POST"})
            if not perr:
                add(
                    "CreateShipment",
                    "shipping-service",
        ...
        if ps:
            ...
            if rand(1, 2) == 1:
                add(
                    "GET /assets",
                    pick(["s3-storage", "cloudflare-cdn"]),
                    latency(30, 150, 15),
                    parent=ps,
                    **{"aws.service": "s3"},
                )
