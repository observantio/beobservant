...
# Observantio Release Restart Script
# Restarts the Observantio production stack from a release bundle.
...
usage() {
  cat <<USAGE
$(colorize "${C_MAGENTA}${C_BOLD}" "Observantio Release Restart")

$(colorize "${C_CYAN}${C_BOLD}" "Usage:")
  ./release/restart.sh [--purge] [--yes] [--help]

$(colorize "${C_CYAN}${C_BOLD}" "Options:")
  --purge   Stop stack and remove named volumes before restart.
...
section "Observantio Restart"
info "Using compose command: ${COMPOSE_CMD[*]}"
info "Project root: ${ROOT_DIR}"

if [[ "$PURGE_VOLUMES" == true ]]; then
  warn "--purge will remove named volumes and permanently delete persisted data."
  if [[ "$AUTO_YES" != true ]]; then
    read -r -p "Type 'purge' to continue: " confirm
    if [[ "$confirm" != "purge" ]]; then
      warn "Restart cancelled. No changes were made."
      exit 0
    fi
  fi
fi

info "Applying host-aware observability tuning..."
"${RUN_OPTIMAL_SCRIPT}"
echo ""
ok "Host tuning complete."
...
info "Pulling latest configured images for this release bundle..."
"${COMPOSE_CMD[@]}" -f docker-compose.prod.yml pull
ok "Images pulled."

info "Starting stack in detached mode..."
"${COMPOSE_CMD[@]}" -f docker-compose.prod.yml up -d
ok "Observantio production stack restarted successfully."

info "Tip: run ${COMPOSE_CMD[*]} -f docker-compose.prod.yml ps to verify service health."
