...
if ! .venv/bin/schemathesis run test-reports/openapi-resolver.json \
  ...
  "${COMMON_ARGS[@]}" \
  --report-junit-path test-reports/schemathesis-resolver.xml; then
  echo "Schemathesis resolver run completed with failures" >&2
  exit 1
fi

echo "Schemathesis resolver run completed"
